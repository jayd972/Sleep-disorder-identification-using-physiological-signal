...
tic
%RBD patients
rbd{1,1} = char('rbd1.txt'); rbd{2,1} = char('rbd2.txt');
rbd{3,1} = char('rbd3.txt'); rbd{4,1} = char('rbd4.txt');
rbd{5,1} = char('rbd5.txt'); rbd{6,1} = char('rbd22.txt'); 
rbd{7,1} = char('rbd7.txt'); rbd{8,1} = char('rbd8.txt');
rbd{9,1} = char('rbd9.txt'); rbd{10,1} = char('rbd10.txt');
rbd{11,1} = char('rbd11.txt'); rbd{12,1} = char('rbd12.txt');
rbd{13,1} = char('rbd13.txt'); rbd{14,1} = char('rbd14.txt');
rbd{15,1} = char('rbd15.txt'); rbd{16,1} = char('rbd16.txt');
rbd{17,1} = char('rbd17.txt'); rbd{18,1} = char('rbd18.txt');
rbd{19,1} = char('rbd19.txt'); rbd{20,1} = char('rbd20.txt');
rbd{21,1} = char('rbd21.txt'); 

% 2. Starting points for all rbd patients.

rbd{1,2} = 4746241; rbd{2,2} = 1;
rbd{3,2} = 1; rbd{4,2} = 3041281;
rbd{5,2} = 15361; rbd{6,2} = 1367041; 
rbd{7,2} = 107521; rbd{8,2} = 138241;
rbd{9,2} = 1; rbd{10,2} = 15361;
rbd{11,2} = 15361; rbd{12,2} = 122881;
rbd{13,2} = 1; rbd{14,2} = 1;
rbd{15,2} = 8294401; rbd{16,2} = 15361;
rbd{17,2} = 61441; rbd{18,2} = 1474561;
rbd{19,2} = 46081; rbd{20,2} = 706561;
rbd{21,2} = 15361; 

%let's start
all_rbd_combined_Hjorth = [];
for ij=1:size(rbd,1)
        patients = rbd;
        ScoreReader_general
        clearvars -except ij file hyp rbd rbd_results all_rbd_combined_Hjorth nor_features_combined
        ...
        if rbd{ij,2}~=0
            record = normalize(record(ch,rbd{ij,2}:end));
        else
...
%Channel_EMG1EMG2_General
    ...
    rbd_results{ij,1} = name;
    rbd_results{ij,2} = combined_Hjorth;
    all_rbd_combined_Hjorth = cat(1,all_rbd_combined_Hjorth,combined_Hjorth);
    
    sprintf('Completed %s',name)

end
 cd('E:\darji')
 save RBD_Results_HjP rbd rbd_results all_rbd_combined_Hjorth
 

disp('RBDlepsy completed successfully.')
toc    
...
%%%%%%%%%%%%%%%%%%  RBDlepsy  %%%%%%%%%%%%%%%%%%%%%%%%%
load('RBD_Results_HjP.mat')
Y = all_rbd_combined_Hjorth;
W_rbd = []; N1_rbd = []; N2_rbd=[]; N3_rbd = []; R_rbd=[];
for i=1:length(Y)
if Y(i,25)==0
W_rbd = cat(1,W_rbd,Y(i,:));
elseif Y(i,25)==1
N1_rbd = cat(1,N1_rbd,Y(i,:));
elseif Y(i,25)==2
N2_rbd = cat(1,N2_rbd,Y(i,:));
elseif Y(i,25)==3
N3_rbd = cat(1,N3_rbd,Y(i,:));
elseif Y(i,25)==5
R_rbd = cat(1,R_rbd,Y(i,:));
end
end
W_rbd(:,25) = 10; N1_rbd(:,25) = 11; N2_rbd(:,25)=12;
N3_rbd(:,25) = 13; R_rbd(:,25)= 15;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
W_normal_vs_rbd = [W_normal; W_rbd];
N1_normal_vs_rbd = [N1_normal; N1_rbd];
N2_normal_vs_rbd = [N2_normal; N2_rbd];
N3_normal_vs_rbd = [N3_normal; N3_rbd];
R_normal_vs_rbd = [R_normal; R_rbd];

temp_LSS_normal=[N1_normal; N2_normal]; temp_LSS_normal(:,25)=0;
temp_LSS_rbd=[N1_rbd; N2_rbd]; temp_LSS_rbd(:,25)=1;
LSS_normal_vs_rbd = [temp_LSS_normal; temp_LSS_rbd];

temp_NREM_normal=[N1_normal; N2_normal; N3_normal]; temp_NREM_normal(:,25)=0;
temp_NREM_rbd=[N1_rbd; N2_rbd; N3_rbd]; temp_NREM_rbd(:,25)=1;
NREM_normal_vs_rbd = [temp_NREM_normal; temp_NREM_rbd];

all_normal_combined_Hjorth(:,25)=0;
all_rbd_combined_Hjorth(:,25)=1;
Allstages_normal_vs_rbd = [all_normal_combined_Hjorth; all_rbd_combined_Hjorth];

cd('E:\darji')
save Normal_vs_RBD_HjP W_normal_vs_rbd N1_normal_vs_rbd N2_normal_vs_rbd N3_normal_vs_rbd R_normal_vs_rbd LSS_normal_vs_rbd NREM_normal_vs_rbd Allstages_normal_vs_rbd
