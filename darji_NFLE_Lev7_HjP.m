...
nfle = {'nfle1.txt', 1981440;'nfle2.txt',1059840; 'nfle3.txt', 15360;
    'nfle4.txt', 614400; 'nfle5.txt',706560; 'nfle7.txt',322560 ;
     'nfle9.txt', 1059840; 'nfle12.txt', 0;'nfle13.txt', 1520640; 
     'nfle14.txt', 1244160; 'nfle15.txt', 384000; 'nfle16.txt', 0; 
    'nfle17.txt',0; 'nfle18.txt', 1075200;'nfle21.txt', 15360; 
    'nfle22.txt', 307200; 'nfle24.txt', 2012160;'nfle28.txt', 0 ; 
    'nfle29.txt', 1966080; 'nfle30.txt', 2519040;'nfle32.txt', 0 ; 
    'nfle34.txt', 184320 ; 'nfle35.txt', 2088960 ;'nfle36.txt', 1658880 ; 
    'nfle37.txt', 983040 ; 'nfle38.txt', 3624960 ;'nfle39.txt', 0 ;};  %'nfle40.txt', 15360

%let's start

all_nfle_combined_Hjorth = [];
for ij=1:size(nfle,1)
        patients = nfle;
        ScoreReader_general
        clearvars -except ij file hyp nfle nfle_results all_nfle_combined_Hjorth nor_features_combined
        ...
        if nfle{ij,2}~=0
            record = normalize(record(ch,nfle{ij,2}:end));
        else
...
%Channel_EMG1EMG2_General
    ...
    nfle_results{ij,1} = name;
    nfle_results{ij,2} = combined_Hjorth;
    all_nfle_combined_Hjorth = cat(1,all_nfle_combined_Hjorth,combined_Hjorth);
    
    sprintf('Completed %s',name)

end
 cd('E:\darji')
 save NFLE_Results_HjP nfle nfle_results all_nfle_combined_Hjorth
 

disp('NFLElepsy completed successfully.')
toc    
...
%%%%%%%%%%%%%%%%%%  NFLElepsy  %%%%%%%%%%%%%%%%%%%%%%%%%
load('NFLE_Results_HjP.mat')
Y = all_nfle_combined_Hjorth;
W_nfle = []; N1_nfle = []; N2_nfle=[]; N3_nfle = []; R_nfle=[];
for i=1:length(Y)
if Y(i,25)==0
W_nfle = cat(1,W_nfle,Y(i,:));
elseif Y(i,25)==1
N1_nfle = cat(1,N1_nfle,Y(i,:));
elseif Y(i,25)==2
N2_nfle = cat(1,N2_nfle,Y(i,:));
elseif Y(i,25)==3
N3_nfle = cat(1,N3_nfle,Y(i,:));
elseif Y(i,25)==5
R_nfle = cat(1,R_nfle,Y(i,:));
end
end
W_nfle(:,25) = 10; N1_nfle(:,25) = 11; N2_nfle(:,25)=12;
N3_nfle(:,25) = 13; R_nfle(:,25)= 15;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
W_normal_vs_nfle = [W_normal; W_nfle];
N1_normal_vs_nfle = [N1_normal; N1_nfle];
N2_normal_vs_nfle = [N2_normal; N2_nfle];
N3_normal_vs_nfle = [N3_normal; N3_nfle];
R_normal_vs_nfle = [R_normal; R_nfle];

temp_LSS_normal=[N1_normal; N2_normal]; temp_LSS_normal(:,25)=0;
temp_LSS_nfle=[N1_nfle; N2_nfle]; temp_LSS_nfle(:,25)=1;
LSS_normal_vs_nfle = [temp_LSS_normal; temp_LSS_nfle];

temp_NREM_normal=[N1_normal; N2_normal; N3_normal]; temp_NREM_normal(:,25)=0;
temp_NREM_nfle=[N1_nfle; N2_nfle; N3_nfle]; temp_NREM_nfle(:,25)=1;
NREM_normal_vs_nfle = [temp_NREM_normal; temp_NREM_nfle];

all_normal_combined_Hjorth(:,25)=0;
all_nfle_combined_Hjorth(:,25)=1;
Allstages_normal_vs_nfle = [all_normal_combined_Hjorth; all_nfle_combined_Hjorth];

cd('E:\darji')
save Normal_vs_NFLE_HjP W_normal_vs_nfle N1_normal_vs_nfle N2_normal_vs_nfle N3_normal_vs_nfle R_normal_vs_nfle LSS_normal_vs_nfle NREM_normal_vs_nfle Allstages_normal_vs_nfle
